...
export default async function DirectorCommissionsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Commissions"
        description="View-only — lead and co-lead commission tracking"
      />
      <Tabs defaultValue="leads">
        ...
        <TabsContent value="leads" className="mt-4">
          {leadRows.length ? (
            <CommissionsTable commissions={leadRows} />
          ) : (
        ...
        <TabsContent value="co-leads" className="mt-4">
          {coLeadRows.length ? (
            <CoLeadCommissionsTable commissions={coLeadRows} />
          ) : (
